...
<div class="p-4 md:p-8 max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Store</h1>
        </div>
        <p class="text-slate-500 ml-13">Update details for <span class="font-bold text-slate-700">{{ $store->name }}</span>.</p>
    </div>
    ...
    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden">
        <form action="{{ route('stores.update', $store) }}" method="POST">
            ...
            <div class="p-8 md:p-10 space-y-8">
                <!-- Name Section -->
                <div>
                    <label for="name" class="block text-sm font-bold text-slate-700 mb-2">Store Name <span class="text-red-500">*</span></label>
                    <input type="text" 
                           name="name" 
                           id="name" 
                ...
                <div>
                    ...
                    <textarea name="address" 
                              id="address" 
                              rows="3" 
                              class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all placeholder:text-slate-400 @error('address') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror">{{ old('address', $store->address) }}</textarea>
                    @error('address')
                        <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><i data-feather="alert-circle" class="w-3 h-3"></i> {{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" 
                              id="description" 
                              rows="3" 
                              class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all placeholder:text-slate-400">{{ old('description', $store->description) }}</textarea>
                </div>
            </div>

            <!-- Footer -->
            <div class="bg-slate-50 p-8 flex items-center justify-end gap-4 border-t border-slate-100">
                <a href="{{ route('stores.index') }}" class="px-6 py-3 font-bold text-slate-600 hover:text-slate-800 transition-colors">
                    Cancel
                </a>
