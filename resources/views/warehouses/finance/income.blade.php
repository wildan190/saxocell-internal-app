...
@section('title', 'Record Income: ' . $warehouse->name)
...
<div class="content-wrapper bg-slate-50/50 min-h-screen pb-20">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
            ...
            <p class="text-slate-500 mt-2 font-medium text-sm">Record revenue for {{ $warehouse->name }}.</p>
        </div>
    </div>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('warehouses.income.store', $warehouse) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" placeholder="e.g. Sales Revenue" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-semibold text-slate-700 focus:outline-none focus:border-blue-500 transition-colors">
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deposit To (Warehouse Account)</label>
